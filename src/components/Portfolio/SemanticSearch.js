...
export default function SemanticSearch() {
	...
	const embedSearchTerm = async () => {
		...
		const response = await fetch('/api/vsearch/embed', {
			method: 'POST',
			body,
		});
		...
		const { answer, chunks } = json;
		if (!answer || !chunks) throw new Error('No answer or chunks returned');
	...
	return (
		<div className="bg-bg2 min-h-screen py-20 px-6 flex flex-col gap-6 items-center">
			...
			<p className="max-w-[800px] text-justify">
				This tool uses OpenAI&apos;s GPT-3 text-embedding-ada model to transform a search term into a
				1,536-element vector. That data is then compared with chunks of data from documents stored in a
				PostgreSQL table using the pg-vector extension. The documents are pre-parsed into roughly 1000-word
				chunks, analyzed by OpenAI, and then stored in the table. When a search is made against the data,
				the back-end makes a remote procedure call to a stored function in the PG database. The function
				returns 3 matches in order of its cosine similarity. Those 3 chunks are concatenated and then
				another API call is made to ChatGPT to digest the information and generate an answer to the question
			...
			<div className="flex flex-col gap-4">
				...
				<ol className="list-decimal">
					<li>What is the Tech Model Railroad Club?</li>
			...
			<h2 className="text-xl">
				PDF Document: <span className="italic">True Hackers</span>
			</h2>
			<p>Upon successful search match, the PDF viewer automatically takes you to the first relevant page.</p>
			<PdfReader file="https://cdn.designly.biz/pdf/hackers.pdf" currentPageNumber={currentPageNumber} />
			<div className="flex flex-col gap-6 pt-10 max-w-[800px]">
				<h2 className="text-2xl text-center">Conclusion</h2>
				<p>
					Thanks to the semantic similarity search, I am able to feed the most relevant passages from the
					body of text to ChatGPT. ChatGPT has the ability to not just find the meaning of the question,
					but also summize the answer based on very limited information.
				</p>
				<p>
					This technology is going to utterly change the world as we know it. Our individual access to
					information and knowledge will be limitless.
				</p>
			</div>
		</div>
	);
